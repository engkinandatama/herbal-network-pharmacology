# %% [markdown]
# # MD Simulation — Phalerin–AGTR1 (RUN 3: MD-ONLY)
...
#
# **Prerequisites:**
#   1. Run 2 output saved as dataset (e.g. "md-phalerin-agtr1-run2")
...
for i in range(Platform.getNumPlatforms()):
    if Platform.getPlatform(i).getName() == "CUDA":
        cuda_ok = True
        break
print(f"  ✅ CUDA: {'ready' if cuda_ok else '⚠️ NOT FOUND'}")

from openff.toolkit import Molecule as OFFMolecule; print("  ✅ openff.toolkit")
import openmmforcefields; print("  ✅ openmmforcefields")
...
RUN2_DATASET = None
for name in ["md-phalerin-agtr1-run2", "md-phalerin-run2",
             "md-phalerin-agtr1-continue"]:
    candidate = Path(f"/kaggle/input/{name}")
    if candidate.exists():
        RUN2_DATASET = candidate
        break

if RUN2_DATASET is None:
    ...
    raise FileNotFoundError(
        "Cannot find Run 2 dataset! Please attach it as a Kaggle dataset.\n"
        "Expected names: md-phalerin-agtr1-run2"
    )
...
if not RUN2_RESULTS.exists():
    ...
    print(f"⚠️ Using flat structure: {RUN2_RESULTS}")

# Also check for original docking dataset (for ligand SDF)
DOCKING_DATASET = Path("/kaggle/input/mahkota-dewa-docking")
# Also check Run 1 dataset as fallback for ligand SDF
RUN1_DATASET = None
for name in ["md-phalerin-agtr1-run1"]:
    candidate = Path(f"/kaggle/input/{name}")
    if candidate.exists():
...
if final_chk.exists():
    ...
    print(f"\n  ✅ Using final.chk ({final_chk.stat().st_size/1e6:.1f} MB) — 75.6 ns")
elif regular_chk.exists():
    checkpoint_file = regular_chk
    print(f"\n  ⚠️ final.chk not found, using checkpoint.chk ({regular_chk.stat().st_size/1e6:.1f} MB) — 70 ns")
else:
...
print(f"  ✅ Topology: {topology_file.name} ({topology_file.stat().st_size/1e6:.1f} MB)")

# Ligand SDF (check multiple locations)
docked_sdf = None
for sdf_candidate in [
    RUN2_RESULTS / "Phalerin_docked.sdf",
    RUN2_RESULTS / f"{SYSTEM_NAME.split('_')[0]}_docked.sdf",
    DOCKING_DATASET / "ligands" / "Phalerin.sdf" if DOCKING_DATASET.exists() else None,
]:
...
if docked_sdf is None and RUN1_DATASET:
    ...
    for sdf_candidate in [
        run1_results / "Phalerin_docked.sdf",
...
print(f"⏱️  TIME SAFETY: Started at {time.strftime('%Y-%m-%d %H:%M:%S')}")
...
assert step_after > 0, f"❌ ABORT: Step is 0! Checkpoint corrupt."
assert step_after > 5_000_000, f"❌ ABORT: Step too low ({step_after:,}). Wrong checkpoint?"
assert step_after < MD_PARAMS["production_steps"], f"❌ Already at 100 ns!"
assert step_after != step_before, f"❌ Checkpoint didn't load!"

print(f"\n  ✅ Checkpoint valid ({ns_done:.1f} ns)")
print(f"  ✅ Need {remaining_ns:.1f} ns more to reach 100 ns")

# Estimate time needed
estimated_hours = remaining_ns / 3.75  # ~3.75 ns/h at 521 steps/s
print(f"  ⏱️  Estimated time: {estimated_hours:.1f}h (budget: {MAX_MD_SECONDS/3600:.1f}h)")
if estimated_hours > MAX_MD_SECONDS / 3600:
    print(f"  ⚠️ May not finish! Will save checkpoint for next run.")
...
print(f"  ⏱️  Time budget: {MAX_MD_SECONDS/3600:.1f}h")
print(f"{'=' * 60}")
...
while simulation.currentStep < total_target:
    ...
    if wall_elapsed > MAX_MD_SECONDS:
        ...
        print(f"\n  ⏱️  TIME LIMIT after {wall_elapsed/3600:.1f}h")
